...
export const CreateTag = (props: ICreateTagProps) => {
  ...
  const onCreateTagSubmit = (formData: IFormFields) => {
    mutateAsync({
      ...
      .then(() =>
        showNotification({ message: t('tags_create_success'), type: 'success' })
      )
      .catch(() =>
        showNotification({ message: t('tags_create_error'), type: 'error' })
      )
      .finally(() => handleClose());
  };
      ...
      >
        ...
        <DialogActions>
          ...
          <LoadingButton
            ...
            autoFocus
            variant="contained"
            onClick={form.handleSubmit(onCreateTagSubmit)}
          >
            {t('save')}
          </LoadingButton>
        </DialogActions>
