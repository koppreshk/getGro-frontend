...
const EditTagDialog = (props: IEditTagProps) => {
  ...
  const onEditTagSubmit = (formValues: { editTagName: string }) => {
    mutateAsync({
      ...
      .catch(() =>
        showNotification({ message: t('tags_edit_error'), type: 'error' })
      )
      .finally(() => handleClose());
  };
  ...
  return (
    <FormProvider {...form}>
      ...
      >
        <DialogTitle id="alert-dialog-title">{t('edit_tag')}</DialogTitle>
        <DialogContent
        ...
        <DialogActions>
          ...
          <LoadingButton
            ...
            autoFocus
            variant="contained"
            onClick={form.handleSubmit(onEditTagSubmit)}
          >
            {t('save')}
          </LoadingButton>
        </DialogActions>
...
export const EditTag = (props: { id: number; name: string; data: ITag[] }) => {
  ...
  return (
    <>
      <CustomIconButton
        iconComponent={<Edit />}
        tooltipProps={{ title: t('edit_tag'), arrow: true }}
