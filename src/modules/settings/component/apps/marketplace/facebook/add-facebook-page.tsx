...
const steps = [
  {
    ...
    description:
      'Map facebook pages to specific groups, enabling you to manage chats.',
  },
  {
...
function PageConfigurations(
  props: Pick<IAddFacebookPageFormProps, 'allQueues' | 'associatedPages'> & {
    isEditing: boolean;
  }
) {
    ...
    <Grid container spacing={3}>
      ...
      <Grid item md={12}>
        <SelectFieldWithLabel
          ...
          rules={{ required: 'This field is required.' }}
          menuOptions={
            props.associatedPages.map((item) => ({
              key: item.page_id.toString(),
  ...
  const renderBasedOnStep = () => {
    switch (activeStep) {
      case 0:
        return (
          <PageConfigurations
            allQueues={props.allQueues}
            associatedPages={props.associatedPages}
            isEditing={isEditing}
          />
        );
      case 1:
