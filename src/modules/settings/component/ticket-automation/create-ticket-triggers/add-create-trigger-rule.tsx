    ...
    switch (activeStep) {
      case 0:
        return (
          <ChooseConditionForm
            data={props.data}
            allAssignments={props.allTriggers}
            mode={props.mode}
            ruleName={props.defaultValues?.ruleName}
          />
        );
      case 1:
  ...
  const onSave = (formData: IAddCreateTriggerRuleFormFields) => {
    onSubmit(formData)
      .then(() => {
        onClose();
        showNotification({
          message: `Successfully ${props.mode === 'edit' ? 'edited' : 'added'} the rules`,
        });
      })
      .catch(() =>
        showNotification({
          message: `Failed to ${props.mode === 'edit' ? 'edit' : 'add'} the rule`,
          type: 'error',
        })
      );
  };
      ...
      >
        {isLastStep || isInBetween ? (
          <Button
            ...
            onClick={isLastStep || isInBetween ? handleBack : onClose}
          >
            {t('back')}
          </Button>
        ) : null}
        <FlexBox
          ...
          width={isLastStep || isInBetween ? 'calc(100% - 95px)' : '100%'}
        >
          <FlexBox gap="20px">
