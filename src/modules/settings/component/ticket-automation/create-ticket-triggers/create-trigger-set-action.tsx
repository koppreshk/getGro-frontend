...
const Condition = (props: ConditionProps) => {
  ...
  const ticketFieldMenuOptions = data.map((item) => ({
    key: item.fieldTriggerActionId.toString(),
    value: item.name,
  }));
  const operatorMenuOptions =
    data
      .find(
        (item) =>
          item.fieldTriggerActionId.toString() ===
          watch(`${fieldArrayName}.${index}.ticketFields`)
      )
      ?.dropdownValues.map((item) => ({
        key: item.id.toString(),
        value: item.name,
      })) || [];
  const assigneeObject = data.find(
    (item) =>
      item.fieldTriggerActionId.toString() ===
      watch(`${fieldArrayName}.${index}.ticketFields`)
  ); //Checking if assignee is selected
  const isAssigneeSelected =
    assigneeObject?.name.toLocaleLowerCase() ===
    'Set Assignee'.toLocaleLowerCase();
  const { pallete, semantics } = useTheme();
  const { t } = useTranslation();

  const getAssigneeMenuOptionsIfExists = () => {
    if (isAssigneeSelected) {
      const selectedOperator = assigneeObject.dropdownValues.find(
        (item) =>
          item.id.toString() === watch(`${fieldArrayName}.${index}.operator`)
      );
      return selectedOperator?.assignedEmployees?.map((item) => ({
        key: item.id.toString(),
  ...
  return (
    <>
      ...
      >
        <FlexBox gap={'32px'} width="calc(100% - 36px)">
          <SelectField
            name={`${fieldArrayName}.${index}.ticketFields`}
            menuOptions={ticketFieldMenuOptions}
            sx={{ width: '33%' }}
            label={t('ticket_fields')}
          />
          <SelectField
            name={`${fieldArrayName}.${index}.operator`}
          ...
          {isAssigneeSelected ? (
            <SelectField
              ...
              menuOptions={getAssigneeMenuOptionsIfExists() || []}
              sx={{ width: '33%' }}
            />
          ) : null}
        </FlexBox>
