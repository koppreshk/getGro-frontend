        ...
        <FlexBox flexDirection="column" gap="15px">
            ...
            <Wrapper flexDirection="column" padding="15px 20px" gap="20px">
                ...
                <Grid container spacing={4}>
                    <Grid item xs={4}>
                        <HeaderLabel >
                            <Typography variant="body2"> {t('first_response_escalation_note')} </Typography>
                        </HeaderLabel>
                        <SelectField name="addEscalation.ftrDuration" menuOptions={escalationTimes.map((data) => ({ key: data.id.toString(), value: data.name }))} sx={{ width: '100%' }} label={t("escalate")} />
                    </Grid>
                    <Grid item xs={8}>
                        <HeaderLabel>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("group")} name="addEscalation.ftrGroup"
                                    options={queueList.map((data) => ({ key: data.id.toString(), value: data.name }))}
                                    placeholder={t("group")} />
                            </Grid>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("agent")} name="addEscalation.ftrAgent"
            ...
            <Wrapper flexDirection="column" padding="15px 20px" gap="20px">
                ...
                <Grid container spacing={4}>
                    ...
                    <Grid item xs={8}>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("group")} name="addEscalation.ntrGroup"
                                    options={queueList.map((data) => ({ key: data.id.toString(), value: data.name }))}
                                    placeholder={t("group")} />
                            </Grid>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("agent")} name="addEscalation.ntrAgent"
                                    options={userList.map((data) => ({ key: data.id.toString(), value: `${data.firstName} ${data.lastName ?? ''}` }))}
                                    placeholder={t("agent")} />
                            </Grid>
                        </Grid>
            ...
            <Wrapper flexDirection="column" padding="15px 20px" gap="20px">
                ...
                <Grid container spacing={4}>
                    <Grid item xs={4}>
                        <HeaderLabel >
                            <Typography variant="body2"> {t('resolution_escalation_note')} </Typography>
                        </HeaderLabel>
                        <SelectField name="addEscalation.resolutionDuration" menuOptions={escalationTimes.map((data) => ({ key: data.id.toString(), value: data.name }))} sx={{ width: '100%' }} label={t("escalate")} />
                    </Grid>
                    <Grid item xs={8}>
                        <HeaderLabel>
                            <Typography variant="body2"> {t('to')} </Typography>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("group")} name="addEscalation.resolutionGroup"
                                    options={queueList.map((data) => ({ key: data.id.toString(), value: data.name }))}
                                    placeholder={t("group")} />
                            </Grid>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("agent")} name="addEscalation.resolutionAgent"
                                    options={userList.map((data) => ({ key: data.id.toString(), value: `${data.firstName} ${data.lastName ?? ''}` }))}
                                    placeholder={t("agent")} />
                            </Grid>
                        </Grid>
