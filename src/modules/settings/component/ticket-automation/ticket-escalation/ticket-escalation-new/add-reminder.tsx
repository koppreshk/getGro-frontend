        ...
        <FlexBox flexDirection="column" gap="15px">
            ...
            <Wrapper flexDirection="column" padding="15px 20px" gap="20px">
                ...
                <Grid container spacing={4}>
                    <Grid item xs={4}>
                        <HeaderLabel >
                            <Typography variant="body2"> {t('first_response_label')} </Typography>
                        </HeaderLabel>
                        <SelectField name="addReminders.ftrDuration" menuOptions={reminderTimes.map((data) => ({ key: data.id.toString(), value: data.name }))} sx={{ width: '100%' }} label={t("duration")} />
                    </Grid>
                    <Grid item xs={8}>
                        <HeaderLabel>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("queue")} name="addReminders.ftrGroup"
                                    options={queueList.map((data) => ({ key: data.id.toString(), value: data.name }))}
                                    placeholder={t("queue")} />
                            </Grid>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("agent")} name="addReminders.ftrAgent"
            ...
            <Wrapper flexDirection="column" padding="15px 20px" gap="20px">
                ...
                <Grid container spacing={4}>
                    <Grid item xs={4}>
                        <HeaderLabel >
                            <Typography variant="body2"> {t('next_response_label')} </Typography>
                        </HeaderLabel>
                        <SelectField name="addReminders.ntrDuration" menuOptions={reminderTimes.map((data) => ({ key: data.id.toString(), value: data.name }))} sx={{ width: '100%' }} label={t("duration")} />
                    </Grid>
                    <Grid item xs={8}>
                        <HeaderLabel>
                            <Typography variant="body2"> {t('send_reminder_to')} </Typography>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("queue")} name="addReminders.ntrGroup"
                                    options={queueList.map((data) => ({ key: data.id.toString(), value: data.name }))}
                                    placeholder={t("queue")} />
                            </Grid>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("agent")} name="addReminders.ntrAgent"
                                    options={userList.map((data) => ({ key: data.id.toString(), value: `${data.firstName} ${data.lastName ?? ''}` }))}
                                    placeholder={t("agent")} />
                            </Grid>
                        </Grid>
            ...
            <Wrapper flexDirection="column" padding="15px 20px" gap="20px">
                ...
                <Grid container spacing={4}>
                    <Grid item xs={4}>
                        ...
                        <SelectField name="addReminders.resolutionDuration" menuOptions={reminderTimes.map((data) => ({ key: data.id.toString(), value: data.name }))} sx={{ width: '100%' }} label={t("duration")} />
                    </Grid>
                    <Grid item xs={8}>
                        <HeaderLabel>
                            <Typography variant="body2"> {t('send_reminder_to')} </Typography>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("queue")} name="addReminders.resolutionGroup"
                                    options={queueList.map((data) => ({ key: data.id.toString(), value: data.name }))}
                                    placeholder={t("queue")} />
                            </Grid>
                            <Grid item xs={6}>
                                <AutocompleteField label={t("agent")} name="addReminders.resolutionAgent"
                                    options={userList.map((data) => ({ key: data.id.toString(), value: `${data.firstName} ${data.lastName ?? ''}` }))}
                                    placeholder={t("agent")} />
                            </Grid>
                        </Grid>
