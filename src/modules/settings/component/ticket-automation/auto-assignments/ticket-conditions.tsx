...
const Condition = (props: ConditionProps) => {
  ...
  const ticketFields = data.map((item) => ({
    key: item.ticketFieldId.toString(),
  ...
  const operators =
    data
      .find(
        (item) =>
          item.ticketFieldId.toString() ===
          watch(`${fieldArrayName}.${index}.ticketFields`)
      )
      ?.operators.map((item) => ({
        key: item.operatorId.toString(),
  ...
  const isInOperatorSelected =
    sourceArray?.operators
      ...
      ?.operatorId.toString() === watch(`${fieldArrayName}.${index}.operator`);
