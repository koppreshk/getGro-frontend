...
import {
  AddRuleContainer,
  EditRuleContainer,
} from 'modules/settings/containers/ticket-automation';
      ...
      <div style={{ height: 'calc(100% - 46px)' }}>
        <Routes>
          <Route
            key="base-route"
            path="/"
            element={<AutoAssignmentsContent {...props} />}
          />
          <Route
            key="add-route"
            path="/add-rule"
            element={<AddRuleContainer allAssignments={props.data} />}
          />
          <Route
            key="edit-route"
            path="/edit-rule"
            element={<EditRuleContainer allAssignments={props.data} />}
          />
        </Routes>
      </div>
    </FlexBox>
  ...
  return (
    <FlexBox padding="20px" flexDirection="column" gap={'20px'} height="100%">
      ...
      >
        ...
        <Button
          variant="contained"
          onClick={toggleAddAutoAssignmentsDrawer}
