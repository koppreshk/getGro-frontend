        ...
        <FormProvider {...methods}>
            <FlexBox padding="20px" width="100%" height="calc(100% - 77px)" flexDirection="column" justifyContent="space-between">
                <Grid container spacing={2}>
                    <Grid item xs={12}>
                        <TextboxField name="statusName" label="Status Name" fullWidth rules={{ required: 'Status name is required' }} />
                    </Grid>
                    <Grid item xs={12}>
                        <SelectField sx={{ width: '100%' }} name="statusCategory" label="Status Category" menuOptions={[{ key: 'online', value: 'Online' }, { key: 'offline', value: 'Offline' }]} fullWidth rules={{ required: 'Selection is required' }} />
                    </Grid>
                    <Grid item xs={12}>
                        <MuiColorInput sx={{ width: '100%' }} label="Status Color" format="hex" value={value} onChange={handleChange} />
                    </Grid>
                </Grid>
