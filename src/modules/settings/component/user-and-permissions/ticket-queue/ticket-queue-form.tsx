...
export const TicketQueueForm = memo((props: ITicketQueueFormProps) => {
  ...
  const onSubmit = React.useCallback(
    async (formvalues: IQueueFormFields) => {
      onFormSubmitHandler(formvalues);
    },
    [onFormSubmitHandler]
  );

  const renderOption: AutoCompleteRenderOptionProps = (
    optionprops,
    option,
    state
  ) => {
    ...
    <FormProvider {...methods}>
      ...
      >
        <Grid container spacing={2}>
          <Grid item xs={6}>
            <TextboxField
              ...
              label={t('queue_name')}
              fullWidth
              rules={{ required: t('queue_name_required') }}
            />
          </Grid>
          <Grid item xs={12}>
            <AutocompleteField
        ...
        <StlyedFlexBox gap="10px" width="100%" justifyContent="flex-end">
          ...
          >
            {isInEditMode ? t('edit_queue') : t('add_queue')}
          </LoadingButton>
        </StlyedFlexBox>
      </FlexBox>
    </FormProvider>
