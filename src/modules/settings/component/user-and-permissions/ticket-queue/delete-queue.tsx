        ...
        mutateAsync({
            ...
            .then((res: IResult) => {
                if (!res.status) {
                    ...
                    showNotification({ message: t('delete_queue_success'), type: 'success' })
                }
            })
            .catch(() => showNotification({ message: t('delete_queue_failed'), type: 'error' }))
    }

    return (
        <>
            <CustomIconButton onClick={toggleDeleteDialogBox} iconComponent={<Delete />} tooltipProps={{ title: t('delete_queue'), arrow: true }} />
