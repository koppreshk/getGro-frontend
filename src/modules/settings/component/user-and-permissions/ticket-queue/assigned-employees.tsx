...
export const AssignedEmployees = (props: Pick<Queue, 'assignedEmployees'>) => {
  ...
  return (
    <FlexBox gap="4px">
      {assignedEmployees.slice(0, 4).map((item, idx) => (
        <EmployeeAvatar key={item.id} item={item} idx={idx} />
      ))}
      {assignedEmployees.length > 4 ? (
        <Tooltip
          arrow
          placement="bottom"
          title={assignedEmployees
            .slice(4)
            .map((rest) => rest.firstName)
            .join(', ')}
        >
          <Avatar
            sx={{
...
const EmployeeAvatar = (props: { item: Employee; idx: number }) => {
  ...
  const { backgroundColor, textColor } = chooseRandomColors(
    firstName + lastName
  );
  return (
    <Tooltip
