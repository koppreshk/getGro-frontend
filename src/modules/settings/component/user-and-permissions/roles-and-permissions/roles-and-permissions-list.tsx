...
const useColumns = (rolesData: IRoles[]) => {
  ...
  const columns = [
    columnHelper.accessor('name', {
      ...
      cell: ({ row: { original } }) => {
        return (
          <RoleNames roleName={original.name} roleType={original.role_type} />
        );
      },
      minSize: 250,
    }),
          ...
          <>
            {original.can_edit_role ? (
              <FlexBox flexDirection="row" gap="5px">
                <CustomIconButton
                  iconComponent={<Edit />}
                  tooltipProps={{ title: t('edit_role') }}
                  onClick={() => navigate(`edit-role?roleId=${original.id}`)}
                />
                <DeleteRolesContainer
                  roleId={original.id}
                  rolesData={rolesData}
                />
              </FlexBox>
              ...
              <CustomIconButton
                ...
                tooltipProps={{ title: t('view_role') }}
                onClick={() => navigate(`view-role?roleId=${original.id}`)}
              />
            )}
          </>
        );
...
const RoleNames = (props: { roleName: string; roleType: string }) => {
  ...
  const { backgroundColor, textColor } = useMemo(
    () => chooseRandomColors(getInitialsByName(roleName)),
    [roleName]
  );
...
export const RolesAndPermissionList = (
  props: IRolesAndPermissionsListProps
) => {
