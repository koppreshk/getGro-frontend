...
import {
  ...
  EditRoleContainer,
  ViewRoleContainer,
} from 'modules/settings/containers/roles-and-permissions';
...
export const RolesAndPermissionsLayout = (
  props: RolesAndPermissionsLayoutProps
) => {
  return (
      ...
      <FlexBox height="calc(100% - 46px)" width="100%">
        <Routes>
          <Route
            key="base-route"
            path="/"
            element={<RolesAndPermissionsLayoutContent {...props} />}
          />
          <Route
            key="add-route"
            path="create-role"
            element={<CreateRoleContainer />}
          />
          <Route
            key="edit-route"
            path="edit-role"
            element={<EditRoleContainer rolesData={props.rolesData} />}
          />
          <Route
            key="view-route"
            path="view-role"
            element={<ViewRoleContainer rolesData={props.rolesData} />}
          />
        </Routes>
      </FlexBox>
    </FlexBox>
...
const RolesAndPermissionsLayoutContent = (
  props: RolesAndPermissionsLayoutProps
) => {
  ...
  return (
    <FlexBox flexDirection="column" gap={'20px'} padding="20px" width="100%">
      <MoreInformation
        information={t('roles_and_permissions_long_description')}
      />
      <FlexBox
        width="100%"
