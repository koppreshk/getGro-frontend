    ...
    <ConfigLinkWrapper onClick={onLinkClick}>
      <OptionWrapper flexDirection="row" gap="10px">
        ...
        <FlexBox flexDirection="column" width="calc(100% - 30px)">
          <Typography variant="h5" className="config-name">
            {' '}
            {label}{' '}
          </Typography>
...
export const ConfigurationOptions = () => {
  ...
  const filteredConfig = useMemo(
    () =>
      searchText
        ? configurations.reduce((acc, curr) => {
            const filteredCats = curr.categoryOptions.filter((catOption) =>
              catOption.label.toLowerCase().includes(searchText.toLowerCase())
            );
      ...
      {filteredConfig.map((data) => (
        ...
        >
          <FlexBox alignItems="center" gap="5px" padding="0 10px">
            {data.categoryIcon()}
            <Typography variant="h4">{data.categoryName}</Typography>
          </FlexBox>
          <GridLayout
            $padding="10px"
            $gridGap="14px 12px"
