...
export const RatingScale = () => {
  ...
  const filteredData =
    selectedRatingValue === '2points'
      ? ratingData.slice(0, 2)
      : selectedRatingValue === '3points'
        ? ratingData.slice(0, 3)
        : ratingData;
  return (
    <>
      <FlexBox flexDirection="column" gap="20px">
        ...
        <RadioGroupField
          name="ratingScale"
          radioOptions={[
            { key: '2points', label: '2 Points' },
            { key: '3points', label: '3 Points' },
            { key: '5points', label: '5 Points' },
          ]}
        />
        <Typography variant="body3">
          Set the rating scale's text (From low to high ratings)
        </Typography>
...
const RatingIndicatorLeftPart = styled.div<{
  ...
  height: 100%;
  width: 15px;
  background-color: ${({ $ratingIndicatorLeftPartBGColor }) =>
    $ratingIndicatorLeftPartBGColor};
...
const StyledTextField = styled(TextboxField)<{
  ...
  && {
    background-color: ${({ $ratingIndicatorRightPartBGColor }) =>
      $ratingIndicatorRightPartBGColor};
    :hover,
    :focus {
      border-color: ${({ $ratingIndicatorRightPartBGColor }) =>
        $ratingIndicatorRightPartBGColor};
    }
  }
`;
      ...
      <FlexBox alignItems="center" gap="20px">
        ...
        <FlexBox height="40px">
          <RatingIndicatorLeftPart
            $ratingIndicatorLeftPartBGColor={ratingIndicatorLeftPartBGColor}
          />
          <StyledTextField
            name={fieldName}
            size="small"
            $ratingIndicatorRightPartBGColor={ratingIndicatorRightPartBGColor}
          />
        </FlexBox>
