...
export const EditRoleContainer = (props: EditRoleContainerProps) => {
    ...
    const onSubmit = (formData: ICreateRoleFormFields) => {
        ...
        }).catch(() => showNotification({ message: 'Failed to edit the role', type: 'error' }))
    }

    const selectedRole = rolesData.find((item) => item.id.toString() === roleId)!;
    const defaultValues = {
        description: selectedRole.description,
        modules: selectedRole.modules.reduce((acc, curr) => {
        ...
        permissions: selectedRole.permissions.reduce((acc, curr) => {
            acc[curr] = true;
            return acc;
        }, {} as {
