...
import {
  IAddExophoneNumber,
  useAddExophoneNumber,
  useFetchExophoneNumbers,
...
export const AddExophoneNumberContainer = (props: {
  togglePopup: () => void;
}) => {
  const { mutateAsync, isLoading: isMutationLoading } = useAddExophoneNumber();
  const { data: exophoneNumData, isLoading: isExophoneNumDataLoading } =
    useFetchExophoneNumbers();
  const { data: allUsersData, isLoading: isAllUsersDataLoading } =
    useFetchAllUsers('all');
