...
import {
  ...
  useEditFacebookPage,
  useFetchAssociatedPages,
} from 'modules/settings/apis/marketplace/facebook';
import { useFetchAllQueues } from 'modules/settings/apis/ticket-automation';
import {
  AddFacebookPageFormBase,
  IAddFacebookPageFormFields,
} from 'modules/settings/component/apps/marketplace/facebook';
import { useTranslation } from 'react-i18next';

export const EditFacebookPageContainer = (props: {
  toggleAddPageDialog: () => void;
  pageDetails: IFacebookConfiguredPages;
}) => {
  ...
  const onSubmit = (formData: IAddFacebookPageFormFields) => {
    mutateAsync({
      ...
      .then((res) => {
        if (res) {
          showNotification({
            message: t('edit_fb_page_success'),
            type: 'success',
          });
          toggleAddPageDialog();
        }
