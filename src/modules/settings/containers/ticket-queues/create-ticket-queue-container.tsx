...
export const CreateTicketQueueContainer = (
  props: ICreateTicketQueueContainerProps
) => {
  const { mutateAsync: createTicketQueue, isLoading: mutationLoading } =
    useCreateTicketQueues();
  const { data, isLoading } = useFetchTicketMetadata();
  const { showNotification } = useNotifications();
  const { t } = useTranslation();
  const submitCreateTicketQueue = React.useCallback(
    (formData: IQueueFormFields) => {
      createTicketQueue({
        ...
        .catch(() =>
          showNotification({ message: t('new_queue_failed'), type: 'error' })
        );
    },
