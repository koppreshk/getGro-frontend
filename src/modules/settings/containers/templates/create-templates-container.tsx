...
export const CreateTemplatesContainer = (props: { toggleAddStatusDrawer: () => void, statusData?: IGenericResponse[] }) => {
    const { mutateAsync: createTemplates, isLoading } = useCreateTemplates();
    const { showNotification } = useNotifications();

    const submitTemplates = React.useCallback((fromValues: ITemplatesFormFields) => {
        createTemplates({
    ...
    return (
        <TemplatesForm mode="create" onFormSubmitHandler={submitTemplates} mutationLoading={isLoading} statusData={props.statusData} />
    )
}