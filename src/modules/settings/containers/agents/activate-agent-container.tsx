...
export const ActivateAgentContainer = (props: { id: number | string }) => {
    ...
    const onActivateHandler = React.useCallback(() => {
        mutateAsync({ id: props.id })
            .then(() => showNotification({ message: t('activate_user_success'), type: 'success' }))
            .catch(() => showNotification({ message: t('activate_user_failure'), type: 'error' }))
    }, [mutateAsync, props.id, showNotification, t]);

    return (
        <CustomIconButton iconComponent={<CheckCircleOutline />} tooltipProps={{ title: t('activate') }} onClick={onActivateHandler} />
    )
}