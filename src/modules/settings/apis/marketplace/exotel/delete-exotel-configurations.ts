...
export const useDeleteExotelConfiguration = () => {
    ...
    const deleteExotelConfig = React.useCallback(() =>
        postData(ExotelConfigurationEndPoint.DELETE_EXOTEL_CONFIG).then((res) => res.json()), [postData]);

    return useMutation({
        mutationFn: deleteExotelConfig,
        mutationKey: ExotelConfigurationQueryKey.DELETE_EXOTEL_CONFIG,
        onSuccess: () => {
            queryClient.invalidateQueries(ExotelConfigurationQueryKey.FETCH_EXOTEL_CONFIG);
        }
    })
};
