...
export const useCreateInstagramConfiguration = () => {
  ...
  const instagramConfiguration = React.useCallback(
    (args: IinstagramConfigurationArgs) =>
      postData(
        `${InstagramConfigurationEndPoint.CREATE_INSTAGRAM_CONFIGURATION}`,
        args
      ).then((res) => res.json()),
    [postData]
  );
...
export const useEditInstagramConfiguration = () => {
  ...
  const instagramConfiguration = React.useCallback(
    (args: Omit<IinstagramConfigurationArgs, 'code'>) =>
      postData(
        `${InstagramConfigurationEndPoint.EDIT_INSTAGRAM_CONFIGURATION}`,
        args
      ).then((res) => res.json()),
    [postData]
  );
