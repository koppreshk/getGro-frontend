...
export const useFetchAvailabilityStatuses = () => {
    ...
    const fetchAvailabilityStatuses = React.useCallback(() => getData(`${AgentsEndPoint.AVAILABILITY_STATUSES}`).then((res) => res.json()), [getData]);
...
export const useFetchCurrentStatus = () => {
    ...
    const fetchCurrentStatus = React.useCallback(() => getData(`${AgentsEndPoint.CURRENT_STATUS}`).then((res) => res.json()), [getData]);
...
export const useUpdateStatus = () => {
    ...
    const updateStatus = React.useCallback((args: { availability_status_id: string | number }) => postData(AgentsEndPoint.UPDATE_STATUS, args).then((res) => res.json()), [postData]);
