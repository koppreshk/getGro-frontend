...
const AppConfig: FC<AppConfigProps> & { Header: FC<HeaderComponentProps>; Body: FC<BodyComponentProps>; TabsPanel: FC<TabsPanelProps> } = ({ children, className }) => {
    return (
        <FlexBox className={className} flexDirection='column' padding='20px' width='100%'>
            <BreadCrumbs />
...
const Header: FC<HeaderComponentProps> = ({ children, className, appDescription, appTitle, appIcon }) => {
    return (
        <StyledFlexbox className={className} justifyContent='space-between' width='100%' padding='20px'>
...
// Define the Body subcomponent
const TabsPanel: FC<TabsPanelProps> = ({ email, lastUpdated, publishedOn, version, website, tabProps }) => {
            ...
            <FlexBox flexDirection="column" width="70%">
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
                        {tabProps.map((tab) => <Tab key={tab.label} label={tab.label} />)}
                    </Tabs>
                </Box>
