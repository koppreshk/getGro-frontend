...
import {
  ISplitTicketsFormFields,
  SplitTicketProps,
  SplitTicketsContent,
} from 'modules/tickets/components/ticket-details/ticket-conversation/email-conversations/more-actions/split-ticket';

interface SplitTicketsContainerProps
    ...
    mutateAsync({
      ...
      .then((res) => {
        if (res.status) {
          onCloseDrawer();
          showNotification({
            message: 'Ticket split was successfull',
            type: 'success',
          });
        } else {
      ...
      .catch(() =>
        showNotification({ message: 'Failed to split tickets', type: 'error' })
      );
  };
