...
import {
  IMergeTicketsFormFields,
  MergeTicketsContent,
} from 'modules/tickets/components/ticket-details/ticket-details-section/ticket-overview/more-actions/merge-ticket/merge-tickets-content';
...
export const MergeTicketsContainer = (props: { onCloseDrawer: () => void }) => {
  ...
  const submitMergeTicketHandler = React.useCallback(
    (formData: IMergeTicketsFormFields & { primaryTicketId: number }) => {
      const {
        addSecondaryLinkInPrimary,
        addSecondaryTicketMessage,
        closeSecondaryTicket,
        sendMail,
        searchTickets,
        primaryTicketId,
      } = formData;
  ...
  return (
    <MergeTicketsContent
      submitMergeTicketHandler={submitMergeTicketHandler}
