...
export const useAttachCustomer = () => {
    ...
    const attachCustomer = useCallback((args: IAttachCustomerArgs) =>
        postData(`${TicketsEndPoint.ATTACH_CUSTOMER}?ticket_id=${args.ticketId}&email=${args.email}&first_name=${args.firstName}&last_name=${args.lastName}`).then((res) => res.json()), [postData]);
