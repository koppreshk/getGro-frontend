...
export const useFetchCallsByTicketId = () => {
    ...
    const getCallsData = React.useCallback(() => getData(`${TicketsEndPoint.FETCH_CALLS_BY_TICKETID}?ticket_id=${ticketId}`).then((res) => res.json()).catch((err) => err), [getData, ticketId]);
