...
export const useTicketsHistory = () => {
    ...
    const getTicketsHistoryData = React.useCallback(() => getData(`${TicketsEndPoint.TICKET_HISTORY}?ticket_id=${ticketId}`).then((res) => res.json()).catch((err) => err), [getData, ticketId]);
