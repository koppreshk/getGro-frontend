...
export const useFetchAllPendingTickets = () => {
    ...
    const fetchAllPendingData = React.useCallback(() => getData(`${TicketsEndPoint.FETCH_ALL_PENDING_TICKETS}?${_pageNumber}items_per_page=${itemsPerPage ?? '10'}${_search}`).then((res) => res.json()), [_pageNumber, _search, getData, itemsPerPage]);
    return useQuery<{ data: ITicketDetails[], total_pages: number }, { message: string }>({
        queryKey: [TicketsQueryKey.FETCH_ALL_PENDING_TICKETS, pageNumber, itemsPerPage, search],
