...
const Content = styled(FlexBox)<{ $isCustomerQuery: boolean }>`
  background-color: ${({ theme, $isCustomerQuery }) =>
    $isCustomerQuery ? theme.pallete.white : '#d9fdd3'};
  padding: 6px;
  border-radius: ${({ $isCustomerQuery }) =>
    $isCustomerQuery ? '0px 6px 6px 6px' : '6px 0px 6px 6px'};

  box-shadow: rgba(0, 0, 0, 0.15) 0px 3px 3px 0px;
`;

const animateClient = css`
  animation: backInLeft 0.3s;
...
const Wrapper = styled(FlexBox)<{ $isCustomerQuery: boolean }>`
  ${({ $isCustomerQuery }) =>
    $isCustomerQuery ? animateClient : animateAgent};
`;
...
export const WhatsAppChatContent = (props: IChatContentProps) => {
  ...
  const { backgroundColor, textColor } = useMemo(
    () =>
      chooseRandomColors(isCustomerQuery ? customerName : agentName || 'NA'),
    [agentName, customerName, isCustomerQuery]
  );
  const agtMsgDeliveryStatus = content.read
    ? 'read'
    : content.delivered
      ? 'delivered'
      : 'sent';
  ...
  return (
    <Wrapper
      ...
      alignItems="center"
      ref={containerRef}
      $isCustomerQuery={isCustomerQuery}
      flexDirection={isCustomerQuery ? 'row' : 'row-reverse'}
    >
      ...
      >
        {content.file_url ? (
          <img src={content.file_url} loading="lazy" />
        ) : null}
        <Typography
          variant="body2"
        ...
        {!isCustomerQuery ? (
          <FlexBox justifyContent="flex-end" gap="5px" alignItems="center">
            ...
            <MessageDeliveryStatuses
              agtMsgDeliveryStatus={agtMsgDeliveryStatus!}
            />
          </FlexBox>
        ) : (
...
const MessageDeliveryStatuses = (props: { agtMsgDeliveryStatus: string }) => {
  ...
  return (
    <Component
      sx={{
        color: agtMsgDeliveryStatus === 'read' ? '#53bdeb' : '#8696a0',
        width: '16px',
        height: '16px',
      }}
    />
