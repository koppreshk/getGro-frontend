...
export const NormalTelephonicDialer = (props: ITelephonicDialerProps) => {
  ...
  const methods = useForm<TelephonicDialerFormFields>({
  ...
  const dial = (formData: TelephonicDialerFormFields) => {
    mutateAsync({
      exophone: formData.exotelAddedNumber,
      ...
      .catch(() =>
        showNotification({ message: 'Failed to place the call', type: 'error' })
      );
  };
  ...
  return (
    <Dialog open={openCallPopUp} maxWidth="xs" fullWidth={true}>
      <FormProvider {...methods}>
          ...
          <FlexBox width="100%" flexDirection="column" gap="20px">
            <FlexBox flexDirection="column" gap={'10px'}>
              ...
              <TextboxField
                ...
                fullWidth
                rules={{ validate: validatePhoneNum }}
              />
            </FlexBox>
          </FlexBox>
        </DialogContent>
        <StyledDialogActions>
          <Button
            onClick={methods.handleSubmit(dial)}
            variant="contained"
            fullWidth
            startIcon={<Call />}
          >
