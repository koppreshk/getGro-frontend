  ...
  return (
    <Card flexDirection="column" gap="16px">
      <IncomingCallHeader flexDirection="row" gap="12px" alignItems="center">
        <Avatar
          sx={{ color: '#ffff', bgcolor: '#a7a7a7', width: 56, height: 56 }}
        ></Avatar>
        <FlexBox flexDirection="column">
          <Typography
            ...
            sx={{ fontSize: '18px' }}
          >
            {incomingCallDetails?.callFromNumber || 'Phone Number'}
          </Typography>
          {isCallAnswered ? (
            <StyledTimer />
          ) : (
            <Typography variant="subheading2" color={pallete.grayVariant4}>
              Incoming call...
            </Typography>
          )}
        </FlexBox>
      ...
      {isCallAnswered ? (
        ...
        >
          ...
          <FlexBox flexDirection="row" gap="9px">
            ...
            <Tooltip
              title={isCallOnHold ? 'Unhold call' : 'Hold call'}
              arrow
              placement="bottom"
            >
              <IconWrapper
        ...
        <IncomingCallFooter flexDirection="row" gap="10px">
          <Button
            ...
            color="error"
            fullWidth
            startIcon={<Phone sx={{ transform: 'rotate(135deg)' }} />}
            onClick={onReject}
          >
            Reject
          </Button>
...
export const IncomingCallMain = () => {
  ...
  return <>{isIncomingCall && <CardComponent />}</>;
};
