...
export const getCallStatusIcon = (callStatus: string) => {
  if (callStatus === 'incoming') {
    return <CallMade sx={{ fill: '#47A83B' }} />;
  } else if (callStatus === 'outgoing') {
    return <CallReceived sx={{ fill: '#47A83B' }} />;
  } else if (callStatus === 'missed') {
    return <CallMissed sx={{ fill: '#C91C2E' }} />;
  }
};
    ...
    <CardWrapper gap="10px">
      ...
      <FlexBox flexDirection="column" gap="5px" width="88%">
        ...
        <FlexBox flexDirection="column">
          <Typography variant="subtitle1" color={pallete.green}>
            {from}
          </Typography>
          <FlexBox flexDirection="row" justifyContent="space-between">
            <FlexBox gap="8px">
              <Typography variant="body2">
                {capitalizeFirstLetter(direction)} call,
              </Typography>
...
export const TelephonicConversationsLayout = (props: {
  data: ICallsByTicketId;
}) => {
  const { data } = props;
  return (
