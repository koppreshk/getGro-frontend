...
export const InsertArticle = (props: { editorType: string; editorValue: string }) => {
    ...
    const setEditorValue = useCallback((formData: ArticleFormFields) => {
        const { articles } = formData;
        const selectedIds = Object.keys(articles).filter((item) => articles ? articles![item] : {});
        const links = data?.filter((item) => selectedIds.includes(item.id.toString()))
        const linksMarkup = (
            <div>
            ...
            <Dialog
                ...
                <FormProvider {...form}>
                    ...
                    <DialogActions sx={{ justifyContent: 'flex-end', padding: '16px' }}>
                        ...
                        <Button variant='contained' endIcon={<ArticleOutlined />} onClick={form.handleSubmit(setEditorValue)}>
                            {t('insert_article')}
                        </Button>
                    </DialogActions>
                </FormProvider>
