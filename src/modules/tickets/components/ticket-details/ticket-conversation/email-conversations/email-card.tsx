    ...
    case 'splitTicket':
      return (
        <SplitTicket
          ...
          showSplitTicketDrawer={showDrawer.splitTicket}
          onCloseDrawer={() => toggleDrawerDisplay('splitTicket')}
        />
      );
    default:
      return <></>;
...
export const EmailCard = (props: IEmailCardProps) => {
  ...
  const { backgroundColor, textColor } = useMemo(
    () => chooseRandomColors(getInitialsByName(from || fromEmail)),
    [from, fromEmail]
  );
  ...
  const onCardClick = () =>
    onSingleEmailCollapseHandler({ messageId, isCollapsed: !isCollapsed });
      ...
      <FlexBox flexDirection="column" gap="12px" justifyContent="center">
        ...
        >
          <FlexBox gap="10px" width="100%">
            <Avatar
              className="no-print"
              sx={{ color: textColor, bgcolor: backgroundColor }}
            >
              {getInitialsByName(from || fromEmail)}
            </Avatar>
            <StyledFlex flexDirection="column">
              <FlexBox justifyContent="space-between">
                <Typography variant="h6">
                  {from || fromEmail}{' '}
                  <span className="print">{`<${fromEmail}>`}</span>
                </Typography>
                <FlexBox alignItems="center" gap="10px">
                  <SubTextValue variant="caption">
        ...
        {!isCollapsed && (
          <InnerHTML dangerouslySetInnerHTML={{ __html: htmlContent }} />
        )}
