...
export const EmailConversations = (props: IEmailConversationsProps) => {
  ...
  const onSendReply = useCallback(
    (formValues: Pick<IEmailFormFields, 'reply'>) => {
      const attachments = formValues.reply.attachments?.selectedFiles.map(
        (item) => ({
          file_name: item.name,
          file_type: item.type,
          file_content: (item.content as string).split(',')[1],
        })
      );
      mutateAsync({
  ...
  return (
    <div style={{ overflow: 'auto', height: '100%' }}>
      <EmailConversationsContainer
        ...
        gap="20px"
      >
        {emailThreads.map((singleEmail, index) => (
          <EmailCard
            key={index}
            emailProps={{ ...singleEmail, subject: props.subject }}
            onSingleEmailCollapseHandler={props.onSingleEmailCollapseHandler}
          />
        ))}
      </EmailConversationsContainer>
      {showReplyEditor ? (
