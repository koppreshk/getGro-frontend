...
export default function CustomerSurveyPage() {
  ...
  const onSubmit = useCallback(
    (formvalues: ICustomerSurveyFields) => {
      mutateAsync({
            ...
            <FlexBox gap="18px">
              {customerSatifaction.map((data) => (
                <StyledRating
                  flexDirection="column"
                  alignItems="center"
                  onClick={() => updateRating(data.rating)}
                  key={data.label}
                >
                  {data.icon()}
