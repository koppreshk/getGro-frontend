...
export const FileUploadDND = (props: FileUploadDNDProps) => {
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    if (droppedFiles.length) {
      ...
      if (file.size > maxSizeInBytes) {
        ...
        const modifiedFiles = droppedFiles
          .slice()
  ...
  const handleFileSelection = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFiles.length) {
      ...
      if (file.size > maxSizeInBytes) {
        ...
        const modifiedFiles = selectedFiles
          .slice()
          .map((file) => ({ file, id: generateId() }));
        setFiles(modifiedFiles);
        setError(null); // Clear any previous error
  ...
  const handleOpenFileDialog: MouseEventHandler<HTMLButtonElement> = (
    ev
  ): void => {
    ev.stopPropagation();
  ...
  return (
    <FlexBox flexDirection="column" height="calc(100% - 160px)" gap={'5px'}>
      ...
      >
        ...
        {files.length ? (
          <UploadedFiles files={files} onDeleteClick={onDeleteClick} />
        ) : null}
      </FlexBox>
