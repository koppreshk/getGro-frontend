...
export function setCookie(cookieName: string, cookieValue: string, expireDays: number, keepMeSignedIn: boolean = false) {
    const date = new Date();
...
export const useCookieStorage = (cookieName: string, defaultValue: null) => {
    const [storedValue, setStoredValue] = useState(() => {
        try {
            ...
            if (value) {
                ...
                setCookie(cookieName, JSON.stringify(defaultValue), DEFAULT_COOKIE_EXPIRATION_VALUE);
                return defaultValue;
            }
        } catch (err) {
            return defaultValue;
    ...
    const setValue = (newValue: object, expireDays: number, keepMeSignedIn?: boolean) => {
        try {
            setCookie(cookieName, JSON.stringify(newValue), expireDays, keepMeSignedIn);
        } catch (err) { /* empty */ }
