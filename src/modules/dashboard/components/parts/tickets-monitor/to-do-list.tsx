...
export const ToDoList = () => {
    ...
    const editTaskText = (id: string, text: string) => {
        setList(list.map((data) =>
            data.id === id ? { ...data, task: text } : data
        ));
    };
    ...
    return (
        <ChartContainer flexDirection="column" gap="10px">
            ...
            <FlexBox width="100%" gap="10px" alignItems="center">
                ...
                <TextField onChange={(e) => setTextValue(e.target.value)}
        ...
        <TaskWrapper justifyContent="space-between" onMouseOver={handleMouseOver} onMouseOut={handleMouseOut} width="100%">
            {isEditing ?
                <TextField
                    ...
                    variant="standard" size="medium" fullWidth
                    onChange={(e) => {
                ...
                <TaskContent>
                    <FlexBox alignItems="center" width="100%">
                        <Checkbox
                            ...
                            size="small"
                        />
                        <StyledTaskDesc variant="body3" $isChecked={isChecked}> {task} </StyledTaskDesc>
                    </FlexBox>
                    {isTaskOpShown &&
                        <TaskOperations flexDirection="row" alignItems="center" height="100%">
                            <CustomIconButton
                                iconComponent={<EditNote fontSize="small" />}
                                tooltipProps={{ title: 'Edit' }} size="small"
                                onClick={toggleEditView} />
                            <CustomIconButton
                                iconComponent={<HighlightOff fontSize="small" />}
                                tooltipProps={{ title: 'Delete' }} size="small"
