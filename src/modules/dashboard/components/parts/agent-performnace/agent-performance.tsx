...
export const AgentPerformance = (props: IAgentPerformanceProps) => {
  ...
  const form = useForm<IAgentPerformanceFormFields>({
    values: {
      ...
      filterValue:
        employees
          .find((item) => item.firstName === user?.name)
          ?.id.toString() ||
  ...
  const menuOptions =
    filterType === 'queue'
      ? queues.map((item) => ({ key: item.id.toString(), value: item.name }))
      : employees.map((item) => ({
  ...
  useEffect(() => {
    if (
      filterType === 'user' &&
      !employees.find((item) => item.id.toString() == form.watch('filterValue'))
    ) {
      form.setValue('filterValue', employees[0].id.toString());
    }
    if (
      filterType === 'queue' &&
      !queues.find((item) => item.id.toString() == form.watch('filterValue'))
    ) {
      ...
      <FlexBox flexDirection="column" gap="15px" height="100%" padding="20px">
        <FlexBox justifyContent="space-between" alignItems="center">
          <FilterContainer alignItems="center">
            ...
            <SelectField
              name="filterValue"
              label={`Selected ${filterType === 'queue' ? 'Queue' : 'User'}`}
