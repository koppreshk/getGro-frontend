...
const CustomerResponse = (props: { csat: CSAT }) => {
  const { negative_rating, neutral_rating, positive_rating, total_rating } =
    props.csat;
  const responses = [
  ...
  return (
    ...
    >
      ...
      {type === 'Positive' ? (
        ...
        <SentimentNeutralOutlined fontSize="large" sx={{ color: '#e1a304' }} />
      ) : (
        <SentimentDissatisfiedOutlined
          fontSize="large"
          sx={{ color: '#f9390f' }}
...
export const CustomerSatifaction = (props: { csat: CSAT }) => {
  ...
  return (
    <StyledContainer gap="20px" flexDirection="column">
      <Typography variant="h5">{t('csat')}</Typography>
      <GridLayout $gridTemplateColumns={'1fr 1fr'} $gridGap={'30px'}>
        <FlexBox
          style={{ borderRight: semantics.standardBorder }}
