...
const StyledContainer = styled(FlexBox)`
  ...
  box-shadow:
    0 4px 8px -2px #1018281a,
    0 2px 4px -2px #18212f0f;
`;
...
const QuickStats = (props: {
  ...
  return (
    <FlexBox
      ...
      gap="10px"
      style={{
        borderLeft: `3px solid ${dashboard.graphBgColor1}`,
        paddingLeft: '15px',
      }}
    >
...
const TicketsCreated = (props: { dateRange: DateRange }) => {
  ...
  const { data: apidata, isLoading } = useFetchSupportMonitoringTicketsCreated(
    form.watch('groupBy'),
    props.dateRange
  );
...
export const TicketStats = (props: { dateRange: DateRange }) => {
  ...
  const { data, isLoading } = useFetchSupportMonitoringStatistics(
    props.dateRange
  );
  ...
  const quickStats1 = [
    ...
    {
      name: t('replies_by_agent'),
      value: data?.replies_by_agents || 0,
    },
    {
      name: t('response_pending'),
  ...
  const quickStats2 = [
    {
      name: t('tickets_closed'),
      value: data?.tickets_closed || 0,
    },
    {
      name: t('replies_by_customers'),
      value: data?.replies_by_customers || 0,
    },
    {
      name: t('resolution_pending'),
      ...
      >
        ...
        <FlexBox width="100%" style={{ minHeight: '340px' }}>
          <FlexBox
            gap="20px"
            width="45%"
            style={{ borderRight: `1px solid ${pallete.grayVariant1}` }}
          >
            {isLoading ? (
              <CenteredCircularProgress />
            ) : (
              <>
                <FlexBox flexDirection="column" gap="60px" width="50%">
                  {quickStats1.map((item) => (
                    <QuickStats key={item.name} item={item} />
                  ))}
                </FlexBox>
                <FlexBox flexDirection="column" gap="60px" width="50%">
                  {quickStats2.map((item) => (
                    <QuickStats key={item.name} item={item} />
                  ))}
                </FlexBox>
              </>
