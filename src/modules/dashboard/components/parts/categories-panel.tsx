...
function CustomTabPanel(props: TabPanelProps) {
  ...
  const selectedTab = dashboardCategories
...
const TabPillWrapper = styled(FlexBox)<{ $isSelected: boolean }>`
  ...
  background-color: ${({ $isSelected, theme }) =>
    $isSelected ? theme.pallete.primaryPurple : '#e5e5ea'};
  color: ${({ $isSelected, theme }) =>
    $isSelected ? theme.pallete.white : 'black'};

  &:hover {
    background-color: ${({ $isSelected, theme }) =>
      $isSelected ? theme.pallete.primaryPurple : '#dcdce1'};
  }
`;
...
const TabPill = (props: ITabPillProps) => {
  ...
  return (
    ...
    >
      <Typography
        variant="caption"
        sx={{ color: `${isSelected ? pallete.white : pallete.grayVariant2}` }}
      >
        {label}
      </Typography>
    </TabPillWrapper>
...
export const DashboardCategoriesPanel = () => {
  ...
  return (
    <>
      <StyledBox>
        <FlexBox
          aria-label="dashboard categories tabs"
          alignItems="center"
          justifyContent="space-between"
        >
          <FlexBox gap="10px" flexDirection="row" alignItems="center">
            <FlexBox alignItems="center" gap="5px" padding="0 12px 0 0">
              ...
              <Typography variant="h4" sx={{ color: pallete.grayVariant2 }}>
                <Trans i18nKey="dashboards" />
              </Typography>
            </FlexBox>
            {dashboardCategories
              .filter((item) => !item.hidden)
