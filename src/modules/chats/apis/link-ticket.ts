...
export const useLinkTicket = () => {
  ...
  const linkTicket = useCallback(
    (args: ILinkTicketArgs) =>
      postData(ChatEndPoint.LINK_TICKET, args).then((res) => res.json()),
    [postData]
  );
...
export const useUnLinkTicket = () => {
  ...
  const linkTicket = useCallback(
    (args: IUnLinkTicketArgs) =>
      postData(ChatEndPoint.UNLINK_TICKET, args).then((res) => res.json()),
    [postData]
  );
