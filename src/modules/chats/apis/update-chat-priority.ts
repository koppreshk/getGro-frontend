...
export const useUpdateChatPriority = () => {
    ...
    const updatePriority = useCallback((args: IUpdatePriorityArgs) =>
        postData(`${ChatEndPoint.UPDATE_CHAT_PRIORITY}?conversation_id=${args.conversationId}&priority_id=${args.priorityId}`).then((res) => res.json()), [postData]);
