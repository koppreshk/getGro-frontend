...
export const CreateLinkDrawerContent = (
  props: CreateLinkDrawerContentProps
) => {
  const {
    priorities,
    allTags,
  ...
  const suggestedTags = formMethods.watch('tags').length
    ? allTags.filter((tag) =>
        formMethods
          .watch('tags')
          .some((associatedTag) => associatedTag.id === tag.id)
      )
    : allTags;
