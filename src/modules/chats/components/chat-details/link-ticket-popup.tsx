...
import { Controller, FormProvider, get, useForm, useFormContext } from "react-hook-form";
import { useTranslation } from "react-i18next";
import { Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Button, Autocomplete, CircularProgress, TextField, debounce } from "@mui/material";
...
export const LinkTicketDialog = (props: LinkTicketDialogProps) => {
    ...
    const { mutateAsync: linkTicket, isLoading: mutationLoading } = useLinkTicket();
    ...
    const onSubmit = (formvalues: ISearchFormFields) => {
        ...
        }).then(() => showNotification({ message: t('link_ticket_success'), type: 'success' }))
            .catch(() => showNotification({ message: t('link_ticket_failure'), type: 'error' }))
        ...
        <FormProvider {...form}>
            <Dialog
                ...
                <DialogActions sx={{ padding: '16px 24px' }}>
                    <Button onClick={toggleLinkTicketDialog} variant='outlined' sx={{ mr: '8px' }}>{t("cancel")}</Button>
                    <LoadingButton isLoading={mutationLoading} type="submit" variant='contained' onClick={form.handleSubmit(onSubmit)}>{t('submit')}</LoadingButton>
                </DialogActions>
            </Dialog>
        </FormProvider>
    )
...
const SearchTickets = (props: { isLoading: boolean, data?: ISearchTickets, onChange: React.ChangeEventHandler<HTMLInputElement> }) => {
                    ...
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField
                                {...params}
                                fullWidth
                                title="Search a ticket by ID or Subject"
                                placeholder="Search a ticket by ID or Subject"
                                size="small"
                                autoFocus
                                error={hasError}
                                required={true}
