...
const Content = styled(FlexBox)<{
  ...
  padding: 6px;
  border-radius: ${({ $isIncomingMessage }) =>
    $isIncomingMessage ? '0px 6px 6px 6px' : '6px 0px 6px 6px'};
...
export const ChatContent = (props: IChatContentProps) => {
  ...
  const { backgroundColor, textColor } = useMemo(
    () =>
      chooseRandomColors(isIncomingMessage ? customerName : replied_by || 'NA'),
    [replied_by, customerName, isIncomingMessage]
  );
  ...
  return (
    <Wrapper
      ...
      flexDirection={isIncomingMessage ? 'row' : 'row-reverse'}
    >
      <Avatar
      ...
      >
        {isIncomingMessage
          ...
          : getInitialsByName(replied_by ?? 'NA')}
      </Avatar>
      <Content
        $isIncomingMessage={isIncomingMessage}
        $source={chatDetails!.chat_source!}
        maxWidth="50%"
