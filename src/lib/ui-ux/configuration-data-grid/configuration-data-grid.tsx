...
export interface IConfigDataGridProps<T>
  extends Pick<TableOptions<T>, 'data' | 'columns' | 'initialState'> {
  isLoading?: boolean;
  totalPages?: number;
...
export const ConfigDataGrid = <T extends object>(
  props: IConfigDataGridProps<T>
) => {
  const {
    columns,
    data,
  ...
  return (
    <DataGridWrapper className={className} height="100%" flexDirection="column">
      <>
        {memoizedData.length ? (
          <>
            {hideTableControls ? null : (
              <TableControls
                totalPages={totalPages}
                enableSerchField={enableSerchField}
              />
            )}
