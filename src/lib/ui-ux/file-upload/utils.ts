...
import { IChangeArgs, IFileInfo, IFileUploadProps, ReadMode } from './file-upload.types';
import { unique } from 'lib/utils/array-utils';
import { generateId } from 'lib/utils';

export function getFileInfo(file: File, readMode: ReadMode): Promise<IFileInfo> {
...
export function getAllFilesInfo(fileList: File[], readMode: ReadMode) {
    return Promise.allSettled(fileList.map((file) => getFileInfo(file, readMode)));
}
...
type FileRepositoryAction = (UpsertFiles | RemoveFile) & { multiple?: boolean, allowDuplicateFiles?: boolean };
    ...
    switch (action.type) {
        case 'UPSERT': {
            if (action.multiple) {
                ...
                return action.allowDuplicateFiles ? concatinatedFiles : unique(concatinatedFiles, i => `${i.name}_${i.type}`);
            }
            const [firstFile] = action.payload;
            return firstFile ? [firstFile] : [];
        }
        case 'REMOVE':
            return action.multiple ? state.filter(i => i.id !== action.payload) : [];
