...
export const TableHeader = <T extends object>(props: ITableHeaderProps<T>) => {
  ...
  const { dragRef, dropRef, options, previewRef } = useDragAndDrop<T>({
    table,
    header,
  });
  ...
  return (
    <TableHeaderWrapper
      ...
      style={{
        ...
        opacity: options.isDragging ? 0.5 : 1,
      }}
    >
      {
        <FlexBox
          ...
          style={{ textAlign: isGroupedHeader ? 'center' : 'unset' }}
        >
