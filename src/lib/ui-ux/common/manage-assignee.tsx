...
export const ManageAssignee = (props: IManageAssigneeProps) => {
  ...
  const selectedAgent = selectedQueue?.assignedEmployees.find(
    (item) => item.id.toString() === assignedAgent?.toString()
  )?.firstName;
      ...
      <FlexBox flexDirection="column" padding="0px 20px" gap={'5px'}>
        ...
        <StyledContainer justifyContent="space-between" onClick={handleClick}>
          <Typography variant="h6">
            {selectedQueue?.name
              ? `${selectedQueue?.name} / ${selectedAgent}`
              : '--'}
          </Typography>
...
const PopoverContent = (
  props: IManageAssigneeProps & { handleClose: () => void }
) => {
  const { data, assignedAgent, assignedQueue, handleClose, onChangeAssignee } =
    props;
