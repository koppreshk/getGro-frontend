...
import { FlexBox, HorizontalSeparator, TypographyName } from "lib/ui-ux"
...
interface IManageStatusProps {
    status: string;
    menuOptions: IGenericResponse[];
    onStatusChange: (statusId: number) => Promise<void>;
}

export const ManageStatus = (props: IManageStatusProps) => {
    const { menuOptions, status, onStatusChange } = props;
    const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
    const [selectedIndex, setSelectedIndex] = useState(menuOptions.findIndex((item) => item.id.toString() === status) || 0);
    const { showNotification } = useNotifications();

    useEffect(() => {
        if (status) {
            setSelectedIndex(menuOptions.findIndex((item) => item.id.toString() === status))
        }
    }, [menuOptions, status]);
    ...
    return (
        <div>
            ...
            <FlexBox flexDirection="column" padding="0px 20px" gap={'5px'}>
                <TypographyName variant="h6"><Trans i18nKey={'status'} /></TypographyName>
                <StyledButton
                    variant="contained"
                    onClick={handleClick}
                    size="small"
                    ...
                    {menuOptions.find((item) => item.id.toString() === status)?.name}
                </StyledButton>
            </FlexBox>
            <Menu
                anchorEl={anchorEl}
