...
export function downloadFromURL(url: string, fileName: string, mimeType: string, skipSafariBrowserCheck?: boolean) {
    if (!skipSafariBrowserCheck && isSafariBrowser()) { // For Safari browser
...
export function saveFile(content: Blob | string, fileName: string, mimeType: string = 'application/octet-stream', skipSafariBrowserCheck: boolean = false): boolean {
    generateURL(content, mimeType).then((url: string) => downloadFromURL(url, fileName, mimeType, skipSafariBrowserCheck));
    return true;
}

const regexToTestIfSafariBrowser = /(Version)\/(\d+)\.(\d+)(?:\.(\d+))?.*Safari\//;
