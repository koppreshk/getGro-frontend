...
export const FileUploadField = (props: IFileUploadFieldProps) => {
    ...
    const _onChange = (args: IChangeArgs, onChange: (...event: any[]) => void) => {
        if (onFileUpload) {
            onFileUpload();
        }
        onChange(args);
    ...
    return (
        <Controller
            ...
            render={({ field: { onChange, value, ref, ...rest } }) => <FileUpload {...props} {...rest} onChange={(args) => _onChange(args, onChange)} initialSelectedFiles={value?.selectedFiles || []} />}
            control={control}
            name={name}
        />
    )
